...
class ReviewController extends Controller
{
    public function add($product_slug)
    {
        ...
        if ($product) {
            ...
            if ($review) {
                ...
                return view('front.reviews.index', compact('product', 'verified_purchase'));
            }
        } else {
    ...
    public function create(Request $request)
    {
        ...
        if ($product) {
            ...
            if ($new_review) {
                return redirect('category/' . $category_slug . '/' . $prod_slug)->with('status', "Thank you for giving your review about the product");
            }
        } else {
            return redirect()->back()->with('status', "The link you followed was broken");
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($user_review != '') {
            ...
            if ($review) {
                ...
                $review->update();
                return redirect('category/' . $review->product->category->slug . '/' . $review->product->slug)->with('status', "Your review updated now");
            } else {
                return redirect()->back()->with('status', "The link you followed was broken");
            }
        } else {
